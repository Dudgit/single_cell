...
class CellDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (x.reshape(self.seq_length, -1), x_input.reshape(self.seq_length, -1), gene_idx, state, input_state)
...
class CellDataset_binary_norman(torch.utils.data.Dataset):
    def __init__(self, adataTarget, adataInput, gene_to_idx, seq_length: int = 32,perturb_key: str = "condition"):
        if scipy.sparse.isspmatrix_csc(adataTarget.X):
            self.target_X = adataTarget.X.tocsr()
        else:
        ...
        self.target_genes = adataTarget.obs[perturb_key].astype(str).values  # Array of strings
        self.target_states = adataTarget.obs.iloc[:,-1].to_numpy(dtype=np.float32)
        self.input_states = adataInput.obs.iloc[:,-1].to_numpy(dtype=np.float32)
        ...
        self.seq_length = seq_length
        self.num_inputs = self.input_X.shape[0]
        self.num_genes = len(gene_to_idx)-1
        self.num_paddings = (self.seq_length - (self.num_genes % self.seq_length)) % self.seq_length
        print(f"Calculated num_paddings: {self.num_paddings} for seq_length: {self.seq_length} and num_genes: {self.num_genes}")
        #self.num_paddings = num_paddings
    ...
    def __getitem__(self, idx):
        ...
        gene_name_str = self.target_genes[idx]
        pert_genes = gene_name_str.split('+')
        
        pert_multi_hot = np.zeros(self.num_genes+1, dtype=np.float32)
        for g in pert_genes:
            if g in self.gene_to_idx:
                pert_multi_hot[self.gene_to_idx[g]] = 1.0
        ...
        return (x.reshape(self.seq_length, -1), x_input.reshape(self.seq_length, -1), pert_multi_hot, state, input_state)
...
def get_loaders(dataset_name,batch_size:int,num_workers:int = 8,ctrl_key:str = "ctrl",perturb_key:str = "condition", seq_length: int = 32):
    ...
    if dataset_name == "norman":
        print("Using CellDataset_binary_norman for dataset:", dataset_name)
        train_set = CellDataset_binary_norman(train_data[train_data.obs[perturb_key] != ctrl_key], ctrl, gene_to_idx,seq_length=seq_length, perturb_key=perturb_key)
        val_set = CellDataset_binary_norman(val_data[val_data.obs[perturb_key] != ctrl_key], ctrl, gene_to_idx,seq_length=seq_length, perturb_key=perturb_key)
        test_set = CellDataset_binary_norman(test_data[test_data.obs[perturb_key] != ctrl_key], ctrl, gene_to_idx,seq_length=seq_length, perturb_key=perturb_key)
    else:
